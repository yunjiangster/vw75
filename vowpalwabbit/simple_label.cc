...
{
  if (all.sd->weighted_examples >= all.sd->dump_interval && !all.quiet && !all.bfgs)    //debug: print update
    {
	...
	sprintf(label_buf,"%8.4f",ld->label);
      ...
      VW::update_dump_interval(all);    //debug: dump interval update
	  fflush(stderr);
    }
}
...
void output_and_account_example(vw& all, example& ec)
{
  ...
  else
  {
    ...
    all.sd->sum_loss += ec.loss;        //debug: accumulate loss
...
{
  output_and_account_example(all, ec);  //debug: bias calculated
